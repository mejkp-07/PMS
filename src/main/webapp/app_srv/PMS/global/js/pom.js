...
PilesOfMoney = function(container,currencyType) {
  this.container = container;
  this.imgPref=currencyType;
  this.bars = [];
...
PilesOfMoney.protocol = 'https:' == document.location.protocol ? 'https:' : 'http:';
...
PilesOfMoney.prototype.draw = function(data, options) {
  ...
  for (var i = 0; i < bars.length; i++) {
    ...
    html.push('<td class="pilesofmoney-bar" style="vertical-align:bottom" width=100 id="', barDomId, '">');
    html.push('<b>', this.escapeHtml(bar.label), '</b><br />'); 
    html.push(this.escapeHtml(bar.formatted)); 
    html.push('<br />');
    html.push('<img src="' + PilesOfMoney.protocol + '/CDSCO/resources/app_srv/cdsco/global/img/money/', imgPref, img, '.png" width="', w, '" height="', h, '" />');
    html.push('</td>');
  }
