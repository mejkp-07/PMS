...
$(document)
	...
	$('#form_role_report').bootstrapValidator({
																		// live:
																		// 'disabled',
										excluded : ':disabled',
										...
										/*	lstBox2:{
												 validators: {
									                  callback: {
									                      message: 'Please select Report between 1 and 30',
					...
					$('#save')
							.click(
									function() {									
										var bootstrapValidator = $("#form_role_report").data('bootstrapValidator');
										...
										if (bootstrapValidator.isValid()) {
											...
											if($('#lstBox2> option').length>0){
												$('#selectedReport').val(($("select#lstBox2 option").map(function() {return $(this).val();}).get()));
												document.getElementById("save").disabled = true;
												$('#form_role_report')[0].submit();
												return true;
											}else{
												sweetSuccess('Error','Please Select atleast one Report');
												return false;
											}	
...
function resetForm() {
	...
	$('#roleName').val('');
	$('#save').text('Save');
	$('#form_role_report').data('bootstrapValidator').resetForm(true);
}
